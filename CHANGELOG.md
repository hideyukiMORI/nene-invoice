...
## [1.0.0] — 公開 GO 待ち（Unreleased）

> **未公開。** この節は v1.0.0 として公開する内容の草案であり、**公開の実行は施主の最終 GO 待ち**。
> 公開時に日付を確定し、`## [1.0.0] — YYYY-MM-DD` に置き換える。

自己ホスト型の **見積・請求・入金管理** OSS（MIT）としての最初の公開リリース（予定）。
