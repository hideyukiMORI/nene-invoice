...
- **デプロイ runbook**（`docs/deployment-runbook-ja.md`）— 稼働中インスタンスへの増分デプロイ手順・
  検証項目・ロールバック・マイグレーションを伴う場合の隔離。

### 変更

- **path モード（`/{slug}/` 配下のテナント）でサイレント再認証を有効化**。リロード後もログイン状態が
  維持されるようになった（従来はテナントごとに 1 度だけの復元だった）。ADR 0022 Amendment 1。
- 英語表示: 取引先の支店サフィックスがカタログ化され、`〜支店` から `{{branch}} Branch` になった
  （日本語表示は変更なし。**PDF は従来どおりサーバ側生成で常に日本語**）。
- アクセシビリティ是正: 候補リスト（コマンドパレット・取引先検索・明細サジェスト）のマークアップ、
  フォーカス離脱の検知、ファイルドロップ領域の実装。**表示は不変**。

### 修正

- PDF の本文フォントを mPDF 同梱の中国語フォント Sun-ExtA から同梱の IPAexゴシックへ（「化」「直」等の字形が日本語になる）。明細表の「No.」見出しの折り返しを解消（#773）
- **path モードで refresh cookie がテナントの scope で発行されていなかった問題**を修正。
  2 回目のリロードでハードログアウトする原因になっていた。
...
> 詳細と順序は [`docs/roadmap.md`](./docs/roadmap.md)（現在地は private 運用ログ）。
