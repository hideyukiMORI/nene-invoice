...
Last updated: 2026-06-28 (定期請求 #519–#523・Clear↔Invoice ライブ化・MFA 設計 #524/#525・ペルソナ評価 R1→R4 を反映)
...
- **定期請求（recurring billing）= ペルソナ評価の「次の一手」** — 永続化(#519)・下書き生成(#520)・CRUD(#521)・管理API＋OpenAPI(#522)・管理UI `/recurring`(#523) すべて **merged**。残: **実行ルート配線（cron/CLI/リクエスト時 due）= #526（P0・最優先）**、自動 issue（採番・適格請求書＝税理士ゲート・未着手）。epic #503 / バックログ epic #518。
...
優先（課金転換に直結する順）:
1. **#526（P0）定期請求の実行ルート配線（cron/CLI/リクエスト時 due）** — R4 最多言及・完成度9割・看板機能が自動で回らない「肝抜け」。採用者(佐藤)も名指し。
2. **#505 銀行入金CSVの真の自動消込** — 渡辺の支払条件・佐藤のゲート（R4 再確認）。
