...
## Active（2026-07-16 セッション）

### 07-11 以降に main へ入っていたもの（本ファイル未反映だった4本）

- **#662** — chore(demo): 入場ログ(層1)を `var/` file sink 化（`error_log` → SSH で可視）。
- **#664** — fix(frontend): `type-check`/`build` を `tsc -b` に是正（**#663 の0ファイル検査バグ**）。`tsconfig.json` は solution 形式（`files: []` + `references`）なのに素の `tsc --noEmit` を呼んでおり、**何も検査せず常に成功**していた（nene-serve #311 と同型）。
- **#665** — chore(standards): active-theme-pointer exemplar アンカーを植栽。
- **#667** — chore(auth): 自前 X-Authorization フォールバックを NENE2 標準 opt-in へ置換（**NENE2 v1.11.0**）。

> ⚠️ **ローカル vendor が腐っていると #667 のテストが6件 error になる**（`Unknown named parameter $enableAuthorizationHeaderFallback`）。`composer.lock` は **v1.11.0** を指すが、ローカル `vendor/` が v1.10.0 のままだと当該 named parameter が実在しないため。CI は lock から `composer install` するので緑。**`composer install` で vendor を lock に同期してから測ること**。

### codegen 出力の drift 是正（#668 / PR #670・横断§4 の invoice 分・統合リナ了承 07-16）

**#657（#626 デモのメール送信プレビュー化）が `openapi.yaml` を編集したが codegen を再実行しておらず、`schema.gen.ts` が 28行 stale だった**（`SendInvoiceEmailPreview` スキーマと 200 応答が不在）。その空白を FE が**同名 interface の手書き**で埋めており、型の正本が2つある状態だった（payout の `tokens.ts` / `api-types.ts` と同型）。**ライブのバグではない**（手書き型の方が narrow なので画面は動く）。

是正: 契約に `const: true` を書いて「always true」を表現（用語レジストリ `terminology.md:134` と backend `SendInvoiceEmailHandler.php:53` が既に宣言していた意味論。これで生成型が `preview: true` になり**型を弱めずに**手書き型を捨てられる）→ 手書き型を生成型からの導出へ → **regen-diff を `check` に導入**（`scripts/check-codegen.mjs`・規約 `03:575` が themegen に発明した形を codegen にも）→ 決定性のため `openapi-typescript` を **exact pin** → knip の `ignore` を削除（実測で不要）。

残: **#669** — knip の `exclude: ["exports","types"]` の narrow 化（実測で12件を沈黙。うち3件が生成型の定型出力 `paths`/`webhooks`/`$defs`＝§4 が「黙らせるしかなかった可能性」と書いていたその現物、残り9件は実コードの未使用 export/type で triage 要）。規約 `05:716` A-5 の条文化は**統合リナの standards patch レーン**。

### 🔴 教訓 — invoice は「**導入≠実行**」を短期間に2回踏んでいる

| | 道具は在った | しかし |
| --- | --- | --- |
| **#663/#664** | `type-check` script | `tsc --noEmit` がプロジェクト無指定で **0ファイルを検査して常に成功**していた |
| **#668** | `codegen` script | **`check` にも CI にも無く**、生成出力が 28行 stale のまま1か月気づかれなかった |

**在ることを、実行されていると数えると偽の緑になる。** 規約 `05:716` A-5 も「生成型が import されているか」しか見ておらず、**鮮度を守らない**のは同じ穴。

**→ ゲートを足したら、変異を入れて実際に落ちることを assert する。** #668 では実践した:
- `openapi.yaml` に probe フィールドを注入 → `codegen:check` が exit=1 になることを確認 → 復旧
- 未接続ファイルを1本置く → knip が exit=1 で検出することを確認（＝`ignore` を外しても検出器が生きている証明）

## Active（2026-07-11 セッション）

- **構造統一性監査 2026-07-11（4製品横断）の invoice 分反映 — ✅ 是正完了（claim 方式のみ設計判断待ち）** — 監査サマリ `docs/audit/2026-07-11-structural-uniformity.md`（#632 merged）。是正の消し込み:
  - #629 build-release.sh NENE2_CONSTRAINT ドリフト → **PR #634 merged**（composer.json から制約を読む方式・実走で nene2 v1.10.0 検収）
  - #630 旧 demo zip 誤配布リスク → **PR #635 merged**（旧 zip 3本削除・`dist/README.md` で配布前ルール明文化・現 main で焼き直し済み）
  - #631-(c) demo LIKE ESCAPE → #636 / **PR #637 merged**（`SqlLike`+`ESCAPE '!'` 還流）
  - #631-(d) RateLimitStorage グローバル束縛 → #638 / **PR #639 merged**（guard 内 inline・deal 形）
  - #631-(e) sweep JST/UTC 回帰テスト → #640 / **PR #641 merged**（`SweepDemoScriptTest`＋PHP_SAPI ガード＋throttle 掃除閾値 window×2）
  - #631-(f) installer PW 8→12 → #642 / **PR #643 merged**
  - #631-(g) installer 自己削除 → #644 / **PR #645 merged**（installer-demo 実機回帰済み）
  - #631-(h) custom_domain unique index → #646 / **PR #647 merged**（migration 追加）
  - #631-(i) claim 方式（案C・vault #141 形）→ **stop-gate 発動・実装見送り**。path tenancy とミドルウェア順序・信頼モデル（URL 正＋OrgGuard fail-close vs claim 正）が矛盾し大改修になるため、設計所見を **#648** に記録して停止（NENE2 `Tenancy` 上流化の設計時に判断）。
...
- **定期請求（recurring billing）= ペルソナ評価の「次の一手」** — 永続化(#519)・下書き生成(#520)・CRUD(#521)・管理API＋OpenAPI(#522)・管理UI `/recurring`(#523) すべて **merged**。**実行ルート配線 #526 = 実装済（this branch `feat/526-recurring-execution-route`）**: Tier B cron `tools/run-recurring.php`（全 org 横断）＋ Tier A 自動（`RecurringDueCheckMiddleware`・`/admin/` リクエスト時に現 org 分を 1日1回 throttle・既定 `RECURRING_INLINE=1`）。**生成は下書きのみ**。残: 自動 issue（採番・適格請求書＝税理士ゲート・未着手）。epic #503 / バックログ epic #518。
- **財務クラスタ ライブ化** — Clear↔Invoice 実接続を契約検証（clear PR #215 で消費者側2バグ修正→6/6 緑）。本番常時化は clear に env（`NENE_INVOICE_API_BASE_URL`/`NENE_INVOICE_BEARER_TOKEN`）設定のみ。Invoice 側は仕様どおりで無変更。
- **MFA（standalone TOTP）** — 設計確定（#524 / `docs/design/mfa-totp.md` / PR #525 merged・Suite ADR 0025 準拠）。実装未着手。**R4 で「決め手に挙げた人ゼロ＝最優先でない」と判明**（#526 等の後）。
- **#492 organizations.external_id（federation 第一歩）** — merged（PR #498）。
- **境界値テスト拡充（#499/PR #501・+152件）** ＋ 登録番号 `\z` 修正（#500/PR #502）— merged。

新規作業は Issue 先行・main 保護のため PR 経由。バックログは epic #518（R2）＋ #526–#528（R4）を参照。

> 旧「NeNe Clear 連携」「Frontend 画面の進め方」「Phase 0+ Backlog」は履歴メモ。記載のフォローアップは概ね merged 済み。
...
- **コンプライアンス gate: ✅ 解除済み（税理士サインオフ取得済み）**。`paid_at`=入金日・外部起票・過入金は Clear 側 client_credit の各点について、**2026-05-30 に税理士サインオフを取得**（記録: `docs/daily/2026-05-30.md` §"Compliance gate honored"、ADR 0009 のゲート条件）。書き込みAPI（#109/#111/#113）はサインオフ取得後にマージ済み。accounting-compliance.md は引き続き拘束で、`paid_at`/外部起票/client_credit の扱いを変更する場合は再度 ADR ＋ 税理士確認が必要。
...
## Next steps（ペルソナ R4 2026-06-28 を反映・優先度順）

> **エコシステム文脈**: clear = 第一の現金の楔／invoice = 財務クラスタの**土台**／Suite = マネージド・クラウド版（体験無料・自社VPS移行設置代行・有料クラウド保証）。戦略 SSOT は `../_work/discussion-log` ＋ `../_work/reports/2026-06-27/`、Invoice 側ペルソナ検証は `docs/research/persona-review-2026-06-27/`。

**ペルソナ R4（全更新提示）= 採用1／検討9／見送り0・支払3。** マネージド・クラウド版の実在で「見送り→検討」の壁は崩れた（8→0）が、**新規採用・新規課金はゼロ**。変換には機能完成が要る（クラウド＝検討の必要条件だが採用/課金の十分条件ではない）。
...
6. 旧 Phase 4 連携: NeNe Records カタログ取込 / NeNe Concierge webhook。認証セッション残（#464 remember-me・タイムアウト／#465 セキュリティレビュー）。

詳細・根拠は引き継ぎ書 `docs/handover/2026-06-28-status-and-next.md`。
