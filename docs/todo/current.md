...
- **構造統一性監査 2026-07-11（4製品横断）の invoice 分反映** — 是正 Issue #629（build-release.sh NENE2_CONSTRAINT ドリフト・即修正級）/#630（旧 demo zip 誤配布リスク）/#631（所見まとめ: LIKE ESCAPE・RateLimitStorage 束縛・sweep 回帰テスト・installer PW 8字/ガード残置・custom_domain index・claim 解決未適用）＋監査サマリ収録 `docs/audit/2026-07-11-structural-uniformity.md`（#632）。
